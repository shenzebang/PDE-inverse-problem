...
import methods.consistency_instances.kinetic_mckean_vlasov as kinetic_mckean_vlasov
...
INSTANCES = {
    ...
    "Kinetic-McKean-Vlasov": kinetic_mckean_vlasov,
}
...
class ConsistencyBased(Method):
    ...
    def create_model_fn(self):
        if self.cfg.pde_instance.name in INSTANCES:
            return INSTANCES[self.cfg.pde_instance.name].create_model_fn(
                self.pde_instance
            )
        else:
    ...
    def test_fn(self, forward_fn, params, rng):
        ...
        if self.cfg.pde_instance.name in INSTANCES:
            return INSTANCES[self.cfg.pde_instance.name].test_fn(
                forward_fn=forward_fn, pde_instance=self.pde_instance, rng=rng
            )
        else:
            raise NotImplementedError
    ...
    def value_and_grad_fn(self, forward_fn, params, rng):
        ...
        if self.cfg.pde_instance.name in INSTANCES:
            return INSTANCES[self.cfg.pde_instance.name].value_and_grad_fn(
                forward_fn=forward_fn,
                params=params,
                data=data,
                rng=rng_vg,
                pde_instance=self.pde_instance,
            )
        else:
            raise NotImplementedError
        ...
        if self.pde_instance.sample_mode == "online":
            ...
            if self.pde_instance.sample_scheme == "exact":
                batch_size_0T = {
                    ...
                    "grid_time": (
                        self.cfg.solver.train.n_time_stamps,
                        self.cfg.solver.train.sample_per_time,
                    ),
                }
                data = {
                    "initial": self.pde_instance.distribution_initial.sample(
                        self.cfg.solver.train.batch_size_init, rng_initial
                    ),
                    "terminal": self.pde_instance.distribution_terminal.sample(
                        self.cfg.solver.train.batch_size_terminal, rng_terminal
                    ),
                    "0T": self.pde_instance.sample_ground_truth(
                        rng_0T, batch_size_0T[self.cfg.solver.train.sample_mode]
                    ),
                    "tau_0T": self.pde_instance.get_time_sample_ground_truth(
                        rng_0T, batch_size_0T[self.cfg.solver.train.sample_mode]
                    ),
                }
                ...
                data["initial"], data["terminal"], data["0T"] = (
                    self.pde_instance.sample_ground_truth(
                        rng_0T, self.cfg.solver.train.batch_size_0T
                    )
                )
            else:
            ...
            n_trajectories, n_time_stamps_0T, _ = self.pde_instance.dataset[
                "0T"
            ].shape  # TODO: check if there is a bug!!!
            ...
            random_sample_index = random.permutation(
                rng_sample, jnp.arange(n_trajectories)
            )[: n_trajectories // interval_sample]
