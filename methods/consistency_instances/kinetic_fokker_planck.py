...
def value_and_grad_fn(forward_fn, params, data, rng, pde_instance: KineticFokkerPlanck):
    ...
    def loss_fn(params):
        loss_initial = jnp.mean(
            jnp.sum(nabla_V_vmap_x(x_initial, params) * v_initial, -1)
        )
        loss_terminal = jnp.mean(
            jnp.sum(nabla_V_vmap_x(x_terminal, params) * v_terminal, -1)
        )
        loss_nabla = jnp.mean(jnp.sum(nabla_V_vmap_x(x_0T, params) ** 2, axis=-1))
        loss_Hessian = jnp.mean(my_prod(x_0T, v_0T, params))
        loss_friction = (
            jnp.mean(jnp.sum(nabla_V_vmap_x(x_0T, params) * v_0T, -1))
        ...
        return (loss_nabla - 2 * loss_Hessian + 2 * loss_friction + loss_nabla_true) + (
            -2 * loss_initial + 2 * loss_terminal
        ) / pde_instance.total_evolving_time

    def loss_ground_truth_fn(params):
        return jnp.mean(
            jnp.sum(
                (nabla_V_true_vmap_x(x_0T) - nabla_V_vmap_x(x_0T, params)) ** 2, axis=-1
            ),
            axis=0,
        )
...
def potential_pretraining(pde_instance: KineticFokkerPlanck, net, params):
    ...
    def pretrain_loss_fn(params, data):
        return jnp.mean(
            jnp.sum(
                (nabla_V(data, params) - perturbation_rate * nabla_V_true(data)) ** 2,
                axis=-1,
            )
        )
